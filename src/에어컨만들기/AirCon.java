...
public class AirCon {
  private   boolean isPower; // 전원 ON/OFF, 두 개 값은 boolean 쓰기
    ...
    // 에어컨에 대해 온도와 바람세기 설정, 설정 이후 조건에 따라 리터 또는 쿨러를 작동
    public void setAirConState() {
        ...
        if (currTemp > setTemp) { // 현재가 더운 상태
            System.out.println("Cooler 가 작동 합니다");
            isCooler = true;
            isHeater = false;
        } else if (currTemp < setTemp) { // 현재가 추운 상태
            System.out.println("Heater 가 작동 합니다.");
            isCooler = false;
            isHeater = true;
        } else {
            isCooler = false;
