...
// Calendar 클래스는 추상클래스 이므로 객체 생성없이 사용, 날짜와 시간과 달리 캘린더 표기법은 각 나라마다 상이하기 때문
...
//        System.out.println(now.get(Calendar.YEAR)+("년")+(Calendar.MONTH)+("월")+(Calendar.DAY_OF_MONTH)+("일")+(" ")+(Calendar.HOUR)+("시")+(Calendar.MINUTE)+("분")+(Calendar.SECOND)+("초"));
