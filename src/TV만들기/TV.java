...
public class TV {
    private boolean isON; // 전원버튼, 접근 제한자 private 설정(같은 클래스 내에서만 접근가능)
    private int channel; // 1 ~ 999
    private int volume; // 0 ~ 1000
TV() {
    isON = false;
    channel = 18;
    volume = 10;
    System.out.printf("기본생성자 호출, 전원 : %%b, 채널 : %ㅇ, 볼륨 %d\n  ", isON, channel, volume);
}
...
// 외부에서 전원 ON/OFF 할 수 있는 기능을 제공(리모컨?)
